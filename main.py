...
@click.option('--emit', '-e', type=click.Choice(['html', 'markdown', 'jsonld', 'analysis']), 
...
def main(rdf_file: Path, emit: str, rdf_format: Optional[str], namespace: Optional[str], full_context: bool, docs_dir: Optional[Path], title: Optional[str]):
    ...
    if rdf_format is None:
        ...
        if rdf_format is None:
            click.echo(f"Cannot auto-detect RDF format for file extension '{suffix}'", err=True)
            click.echo("Please specify --rdf-format with one of: turtle, xml, nt, n3, json-ld, trig, nquads", err=True)
    ...
    try:
        ...
        click.echo(f"Try specifying a different --rdf-format (current: {rdf_format})", err=True)
        raise click.ClickException(f"Failed to parse {rdf_file}")
    ...
    if emit == 'html':
        ...
        click.echo(f"Unknown output format: {emit}. Supported formats are: html, markdown, jsonld, analysis.")
